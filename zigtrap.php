...
/*
Plugin Name: ZigTrap
Plugin URI: https://www.zigpress.com/plugins/zigtrap/
Version: 1.0
...
if (!class_exists('zigtrap')) {
	...
	class zigtrap
	{
		...
		public function __construct() {
			$this->fieldname = 'notes5';
			global $wp_version;
			if (version_compare($wp_version, '4.0', '<')) { wp_die('ZigTrap requires WordPress 4.0 or newer. Please update your installation.'); }
		...
		public function action_rightnow_end() {
			?>
			<p class="zigtrap_dashboard_stats"><a href="https://www.zigpress.com/plugins/zigtrap/"><strong>ZigTrap</strong></a> has prevented <?php echo $this->options['TotalTrapped']?> spam comments from being stored.</p>
			<?php
		}
		...
		public function filter_pre_comment_approved($approved) {
			if (!empty($_POST[$this->fieldname])) {
				...
				update_option("zigtrap_options", $this->options);
				wp_die('<a href="https://www.zigpress.com/plugins/zigtrap/">ZigTrap</a> triggered - comment is spam.');
			}
			return $approved;
		}
		...
		public function filter_plugin_row_meta($links, $file) {
			...
			if ($file == $plugin) return array_merge($links, array('<a target="_blank" href="https://www.zigpress.com/donations/">Donate</a>'));
			return $links;
		}
